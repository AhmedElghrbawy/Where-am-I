...
(define (intersection-points circles)
  (if (<= (length circles) 1) '()
      (append 
	(apply append (map (lambda (c2) (intersect (car circles) c2)) (cdr circles)))
...
(define (distance-product p points)
  (apply * 
	 (map (lambda (op) (if (same-point? p op) 1
			       (dist p op))) points)))

(define (rate-points points)
  (map (lambda (p) (list (distance-product p points) p)) points))


(define (partition seq piv comp)
  (if (null? seq) '(() ())
      (let ((res (partition (cdr seq) piv comp)))
	(if (comp (car seq) piv) 
            (list (cons (car seq) (car res)) (cadr res))
            (list (car res) (cons (car seq) (cadr res)))))))


(define (quicksort seq comp)
  (if (<= (length seq) 1) seq
      (let ((parts (partition seq (car seq) comp)))
	(append (quicksort (car parts) comp) (list (car seq)) (quicksort (cdadr parts) comp)))))


(define (sort-points rated-points)
  (quicksort rated-points (lambda (rp1 rp2) (< (car rp1) (car rp2)))))


(define (clumped-points points)
  (prefix-of-list 
    (map cadr (sort-points (rate-points points))) 
    (quotient (length points) 2)))


(define (average-point points)
  (let ((avg-p 
	  (make-pt (/ (apply + (map x points)) (length points))
		   (/ (apply + (map y points)) (length points)))))
    (list (distance-product avg-p points) avg-p)))
...
(define (where-am-i distances stars)
  (quicksort (map best-estimate (all-guesses distances stars)) 
	     (lambda (avp1 avp2) (< (car avp1) (car avp2)))))
